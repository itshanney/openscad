...
// Mounting holes
hole_dia      = 8;      // mm
hole_spacing  = 177;    // mm, center to center
hole_from_top = 21;     // mm, from top edge to hole center
...
lip_thick       = 2;    // mm, thickness of the lip

// Derived border widths
border_x = (frame_length - plate_length) / 2;
border_y = (frame_width  - plate_width)  / 2;

// Clip everything to the outer frame boundary so the lip corners match the frame radius
intersection() {
    union() {
        difference() {
    ...
    // Outer boundary — clips the lip's square corners to the frame's corner radius
    hull() {
        translate([corner_radius, corner_radius, 0])
            cylinder(r=corner_radius, h=frame_thick, $fn=64);
        translate([frame_length - corner_radius, corner_radius, 0])
